...
interface BookingFormData {
  ...
  endDate: string;
  amount: number;
  depositAmount: number; // Changed from string to number
  notes: string;
  status: string;
  address: Address;
  nextOfKin: NextOfKin;
...
export function BookingForm({ properties, onSuccess }: BookingFormProps) {
  ...
  const [formData, setFormData] = useState<BookingFormData>({
    ...
    endDate: "",
    amount: 0,
    depositAmount: 0, // Changed to number
    notes: "",
    status: "confirmed",
    address: {
      street: "",
  ...
  // Handle input change with proper type conversion for numeric fields
  ...
  const handlePropertySelect = (propertyId: string) => {
    ...
    if (selectedProperty) {
      setFormData({
        ...
        depositAmount: selectedProperty.depositAmount || 0,
      });
    ...
    try {
      ...
      if (checkAvailability) {
        ...
        if (!(checkAvailability as AvailabilityResult).available) {
          ...
          toast({
            variant: "destructive",
            description: `Property is already booked from ${new Date(
              conflict!.startDate
            ).toLocaleDateString()} to ${new Date(conflict!.endDate).toLocaleDateString()} by ${
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const bookingId = await addBooking({
        ...
        depositAmount: Number(formData.depositAmount), // Ensure it's a number
